...
with the given input at any time.

In the discussion of the technical machinery of \gls{genai} models,
particularly the \glspl{llm} in Chapter~\ref{cha:ai}, we observed their
tendency to form an overarching latent distribution of the training
data. Every output is generated under the gravitational pull of this
distribution, which shapes meaning at any given prompt.

Although there is a rich history of theory
about how humans perceive the world, there is considerable amount of
...
\citeauthor{forrester1971} \parencite*{forrester1971}  summarises, \textit{nobody in his head imagines all the world, government or country [...] (he) has only selected concepts, and relationships between them, and uses those to represent the real system}.

While when analysing the \gls{genai} models, we often focus on how they parse
the given data into meaningful creations, for the \gls{ai} development this
representational fabric plays a much larger role, as one of the most prominent
questions in this area right now, \textit{how the capabilities of the \gls{ai} models can be generalized to interact with the world and deal with the problems they have never encountered before} (see \cite[]{lecun2022}). This problematic holds an important position for the future of robotics or generally for the advancements in the context of \gls{agi} in the future.
